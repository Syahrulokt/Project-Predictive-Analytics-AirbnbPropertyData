...
xx = ['review_scores_rating', 'review_scores_accuracy', 'review_scores_cleanliness', 'review_scores_checkin', 'review_scores_communication', 'review_scores_location', 'review_scores_value']

XX_train, XX_test, yy_train, yy_test = train_test_split(data[xx], data['revenue'], test_size=0.2, random_state=42)
...
XXX_train, XXX_test, yyy_train, yyy_test = train_test_split(X, target_revenue, test_size=0.2, random_state=42)

model3.fit(XXX_train, yyy_train)

model4 = LinearRegression()

XXXX_train, XXXX_test, yyyy_train, yyyy_test = train_test_split(X, target_rating, test_size=0.2, random_state=42)

model4.fit(XXXX_train, yyyy_train)
...
yyyy_pred = model4.predict(XXXX_test)
evaluate_model(yyyy_test, yyyy_pred, model_name="Linear Regression - Revenue Prediction")
...
log_result("Linear Regression - Rating Prediction", yyyy_test, yyyy_pred)
display(results)
...
yyyy_true = yyyy_test.squeeze()

residuals = yyyy_true - yyyy_pred.flatten()

plt.figure(figsize=(10, 6))
sns.scatterplot(x=yyyy_pred.flatten(), y=residuals)
...
plt.figure(figsize=(10, 6))
sns.scatterplot(x=yyyy_true, y=yyyy_pred.flatten())
plt.plot([yyyy_true.min(), yyyy_true.max()], [yyyy_true.min(), yyyy_true.max()],
         '--', color='red')
...
residuals_rating = yyyy_test['review_scores_rating'] - yyyy_pred.flatten()

plt.figure(figsize=(10, 6))
sns.scatterplot(x=yyyy_pred.flatten(), y=residuals_rating)
plt.axhline(0, linestyle='--', color='red')
...
plt.figure(figsize=(10, 6))
sns.scatterplot(x=yyyy_test['review_scores_rating'], y=yyyy_pred.flatten())
plt.plot([yyyy_test['review_scores_rating'].min(), yyyy_test['review_scores_rating'].max()],
         [yyyy_test['review_scores_rating'].min(), yyyy_test['review_scores_rating'].max()],
         '--', color='red')
...
"""#6.&nbsp;Jawaban Business Understanding
...
display_results(y_test, model1.predict(X_test))
...
coefficients['coefficient'] = model1.coef_
...
display_results(yy_test, model2.predict(XX_test))

coefficients = pd.DataFrame(xx, columns=['regressor'])
coefficients['coefficient'] = model2.coef_
plt.bar(coefficients.sort_values('coefficient', ascending=False)['regressor'], coefficients.sort_values('coefficient', ascending=False)['coefficient'])
plt.xticks(rotation=90)
plt.show()
...
display_results(yyy_test, model3.predict(XXX_test))
...
coefficients['coefficient'] = model3.coef_.T
plt.bar(coefficients.sort_values('coefficient', ascending=False)['regressor'], coefficients.sort_values('coefficient', ascending=False)['coefficient'])
plt.xticks(rotation=90)
plt.show()
...
display_results(yyyy_test, model4.predict(XXXX_test))
...
coefficients = pd.DataFrame(X.columns.to_list(), columns=['regressor'])
coefficients['coefficient'] = model4.coef_.T
plt.bar(coefficients.sort_values('coefficient', ascending=False)['regressor'], coefficients.sort_values('coefficient', ascending=False)['coefficient'])
plt.xticks(rotation=90)
plt.show()
